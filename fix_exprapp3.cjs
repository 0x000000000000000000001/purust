...
// Find the App fn args block
const startIdx = code.indexOf('  App fn args ->');
const nextCase = code.indexOf('  UncurriedEffectApp fn args ->', startIdx);
const appBlock = code.substring(startIdx, nextCase);

const newAppBlock = `  App fn args -> 
...
code = code.substring(0, startIdx) + newAppBlock + code.substring(nextCase);
