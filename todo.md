...
La [réutilisation d’un enfant de record](../../altbak.pub-purust/scratch/rust-record-child-20260909/REPORT.md) donne désormais **Records 0,861 → 0,767 ms (−10,9 %)** sur cinq paires, avec **20 003 → 10 003 allocations/libérations**. Le total **19,735 → 19,733 ms** reste stable dans la dispersion ; **22 tests de génération, 13 tests TAST et 14 résultats du runner** passent. Un seul enfant immédiat est réutilisé ; le dernier niveau conserve sa copie.
...
- [ ] **Prochain bébé Records : réutiliser le dernier niveau.** La copie de `d` explique encore **10 000 allocations pour 10 000 itérations**. Prototyper une extension à ce niveau, vérifier les anciennes versions partagées à chaque profondeur, puis compter et mesurer avant d’intégrer un chemin plus profond. Les trois allocations initiales constituent un plancher pour ce cas unique ; aucun gain temporel de cette extension n’est encore confirmé. Les layouts unboxés B21/B23 restent une piste distincte.
