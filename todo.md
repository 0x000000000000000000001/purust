...
- [ ] **Step 1 (Enums Natifs pour ADTs) :** Utiliser les métadonnées de `dataDecls` (fournies par le TAST) pour générer des `enum` Rust structurés et stricts, remplaçant la sérialisation systématique dans des `Value::Array`. C'est la prochaine étape pour effondrer le temps d'exécution des arbres (Red-Black Tree, AST).
