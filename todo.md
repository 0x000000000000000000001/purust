...
- [ ] Propager le contexte des enums en valeur à tous les modules et aux signatures FFI dans `Main` ; vérifier les échanges entre modules et les conversions aux frontières dynamiques et FFI, puis activer ce chemin dans le runner.
...
**14 tests de génération + 1 test TAST passent**, et `bin/rust/run -c` donne les **14 résultats attendus**. Les **301 sources Rust du runner restent identiques** : son entrée par défaut conserve l'ABI actuelle jusqu'à la propagation du contexte aux modules et FFI. Le nouveau chemin est exercé explicitement dans le cas minimal ; aucun gain de temps supplémentaire n'est attribué au runner à cette étape. [Rapport et preuves](../../altbak.pub-purust/scratch/rust-value-enums-local-20260908/REPORT.md).
