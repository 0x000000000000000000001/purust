...
- [x] Préserver cette spécialisation dans la boucle et émettre un accumulateur `i64`, avec conversions uniquement aux frontières qui les nécessitent. PBO conserve le `ForAll` de la déclaration et applique les arguments TAST à une copie de l'implémentation lors de l'inlining ; les instanciations ouvertes ou incomplètes restent génériques.
