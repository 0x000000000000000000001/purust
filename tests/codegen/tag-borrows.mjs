...
const colorType = new ADT('Color', ['TagBorrows', 'Color'], []);
...
const bindings = [returning('direct', tree), returning('wrapped', wrapped),
  ...
  // No operand annotation: only the constructor metadata identifies Tree.
  returning('opaque', tree, Any.value)];
for (const [name, constructor] of [['opaqueEmpty', 'Empty'], ['opaqueColor', 'Red']]) {
  bindings.push(new Tuple(name, new Typed(new Func([Any.value], BooleanType.value),
    new Abs([param('tree', 0)], tag(constructor, tree)))));
}
bindings.push(new Tuple('opaqueCall', new Typed(new Func([new Func([Unit.value], Any.value)], BooleanType.value),
...
arities = insert(ordString)('TagBorrows_Red')(colorType)(arities);
arities = insert(ordString)('TagBorrows_Black')(colorType)(arities);
const core = {
  name: 'TagBorrows', classDecls: [],
  dataDecls: [
    { name: 'Tree', constructors: [{ name: 'Empty', fields: [] }, { name: 'Full', fields: [Int.value] }] },
    { name: 'Color', constructors: [{ name: 'Red', fields: [] }, { name: 'Black', fields: [] }] },
  ],
};
...
fn main() {
    ...
    assert!(TagBorrows_opaqueColor(Value::Class(Rc::new(Color::Red))));
    assert!(!TagBorrows_opaqueColor(Value::Class(Rc::new(Color::Black))));
    ...
    assert!(TagBorrows_opaqueCall(Func1::Shared(Rc::new(move |()| {
        observed.set(observed.get() + 1);
        Value::Class(Rc::new(Rc::new(Tree::Full(7))))
    }))));
    assert_eq!(calls.get(), before + 1, "an opaque operand must be evaluated exactly once");
    for tree in [&full, &empty] {
        let observed = calls.clone();
...
try {
  ...
  console.log('Tag tests preserve shared parents, opaque sum/value-enum checks, conversions and single evaluation.');
