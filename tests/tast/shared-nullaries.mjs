...
const fixture = fileURLToPath(new URL('fixtures/shared-nullaries/', import.meta.url));
const prelude = fileURLToPath(new URL('../../../purust-prelude/src/', import.meta.url));
const directory = mkdtempSync(join(tmpdir(), 'purust-shared-nullaries-'));
...
try {
  ...
  const tast = JSON.parse(readFileSync(join(output, 'SharedNullaries/corefn.json'), 'utf8'));
  ...
  run(process.execPath, ['--stack-size=65536', join(root, 'bin/purust.js'),
    '--source', output, '--out', rust, '--main', 'SharedNullaries']);
  ...
  run('rustc', ['--edition=2021', '-C', 'opt-level=1', join(fixture, 'checks.rs'), '-o', binary,
    '-L', `dependency=${directory}`, ...['purust_core', 'Purs_SharedNullaries', 'Purs_OtherNullaries']
