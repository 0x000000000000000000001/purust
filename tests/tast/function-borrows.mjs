...
const fixture = fileURLToPath(new URL('fixtures/function-borrows/', import.meta.url));
...
try {
  ...
  run(process.env.PURS ?? 'purs', ['compile', join(prelude, '**/*.purs'),
    join(fixture, 'FunctionBorrows.purs'), '--codegen', 'corefn', '--output', output]);
  const tast = JSON.parse(readFileSync(join(output, 'FunctionBorrows/corefn.json'), 'utf8'));
  ...
  run(process.execPath, ['--stack-size=65536', join(root, 'bin/purust.js'),
    '--source', output, '--out', rust, '--main', 'FunctionBorrows']);
  ...
  run('rustc', ['--edition=2021', '-C', 'opt-level=1', join(fixture, 'checks.rs'), '-o', binary,
    '-L', `dependency=${directory}`, ...['purust_core', 'Purs_FunctionBorrows']
