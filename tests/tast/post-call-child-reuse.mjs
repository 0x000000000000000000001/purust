...
const fixture = fileURLToPath(new URL('fixtures/post-call-child-reuse/', import.meta.url));
const prelude = fileURLToPath(new URL('../../../purust-prelude/src/', import.meta.url));
const directory = mkdtempSync(join(tmpdir(), 'purust-post-call-child-reuse-'));
...
try {
  ...
  run(process.env.PURS ?? 'purs', ['compile', join(prelude, '**/*.purs'),
    join(fixture, 'PostCallChildReuse.purs'), '--codegen', 'corefn', '--output', output]);
  const tast = JSON.parse(readFileSync(join(output, 'PostCallChildReuse/corefn.json'), 'utf8'));
  ...
  run(process.execPath, ['--stack-size=65536', join(root, 'bin/purust.js'),
    '--source', output, '--out', rust, '--main', 'PostCallChildReuse']);
  const generated = readFileSync(join(rust, 'Purs_PostCallChildReuse/src/lib.rs'), 'utf8');
  const bodies = new Map(generated.split(/^pub fn /m).slice(1).map(body =>
    [body.match(/^PostCallChildReuse_(\w+)\(/)?.[1], body]));
  const postCallPath = /purust child call: post-call fields/;
  for (const name of ['advanceLeft', 'advanceRight']) {
    assert.match(bodies.get(name), postCallPath, `Inspect the returned child in ${name}`);
  }
  for (const name of ['throughCallback', 'bothChildren']) {
    assert.doesNotMatch(bodies.get(name), postCallPath, `Keep the ordinary path in ${name}`);
  }
  ...
  run('rustc', ['--edition=2021', '-C', 'opt-level=1', join(fixture, 'checks.rs'), '-o', binary,
    '-L', `dependency=${directory}`, ...['purust_core', 'Purs_PostCallChildReuse']
