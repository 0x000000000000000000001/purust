...
const fixture = fileURLToPath(new URL('fixtures/record-borrows/', import.meta.url));
const prelude = fileURLToPath(new URL('../../../purust-prelude/src/', import.meta.url));
const directory = mkdtempSync(join(tmpdir(), 'purust-record-borrows-'));
...
try {
  ...
  const tast = JSON.parse(readFileSync(join(output, 'RecordBorrows/corefn.json'), 'utf8'));
  ...
  run(process.execPath, ['--stack-size=65536', join(root, 'bin/purust.js'),
    '--source', output, '--out', rust, '--main', 'RecordBorrows']);
  const generated = readFileSync(join(rust, 'Purs_RecordBorrows/src/lib.rs'), 'utf8');
  const bodies = new Map(generated.split(/^pub fn /m).slice(1).map(body =>
    [body.match(/^RecordBorrows_(\w+)\(/)?.[1], body]));
  const borrowedScalar = /purust record: borrowed scalar/;
  for (const name of ['readRoot', 'readMiddle', 'readDeep', 'readNumber', 'readBoolean', 'readChar', 'readCollision',
    'notFlag', 'negateTally', 'negateRatio', 'numberPlus', 'numberBefore', 'charBefore', 'andThen', 'orElse', 'branchRead']) {
    assert.match(bodies.get(name), borrowedScalar, `Borrow the scalar path in ${name}`);
  }
  for (const name of ['ownedMiddle', 'ownedLeaf', 'ownedString', 'ownedFunction', 'callerOpaque', 'orderedOpaque']) {
    assert.doesNotMatch(bodies.get(name), borrowedScalar, `Keep the owned path in ${name}`);
  }
  ...
  run('rustc', ['--edition=2021', '-C', 'opt-level=1', '-C', 'overflow-checks=yes', join(fixture, 'checks.rs'), '-o', binary,
    '-L', `dependency=${directory}`, ...['purust_core', 'perceus_ptr', 'Purs_RecordBorrows']
