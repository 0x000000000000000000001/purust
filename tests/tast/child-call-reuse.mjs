...
const fixture = fileURLToPath(new URL('fixtures/child-call-reuse/', import.meta.url));
const prelude = fileURLToPath(new URL('../../../purust-prelude/src/', import.meta.url));
const directory = mkdtempSync(join(tmpdir(), 'purust-child-call-reuse-'));
...
try {
  ...
  run(process.env.PURS ?? 'purs', ['compile', join(prelude, '**/*.purs'),
    join(fixture, 'ChildCallReuse.purs'), '--codegen', 'corefn', '--output', output]);
  const tast = JSON.parse(readFileSync(join(output, 'ChildCallReuse/corefn.json'), 'utf8'));
  ...
  run(process.execPath, ['--stack-size=65536', join(root, 'bin/purust.js'),
    '--source', output, '--out', rust, '--main', 'ChildCallReuse']);
  const generated = readFileSync(join(rust, 'Purs_ChildCallReuse/src/lib.rs'), 'utf8');
  const bodies = new Map(generated.split(/^pub fn /m).slice(1).map(body =>
    [body.match(/^ChildCallReuse_(\w+)\(/)?.[1], body]));
  const childCallPath = /purust child call: retained fields/;
  for (const name of ['walkLeft', 'walkRight']) {
    assert.match(bodies.get(name), childCallPath, `Specialize the direct child call in ${name}`);
  }
  for (const name of ['throughCallback', 'ordered', 'bothChildren', 'aliasChild', 'walkChecked']) {
    assert.doesNotMatch(bodies.get(name), childCallPath, `Keep the ordinary path in ${name}`);
  }
  ...
  run('rustc', ['--edition=2021', '-C', 'opt-level=1', join(fixture, 'checks.rs'), '-o', binary,
    '-L', `dependency=${directory}`, ...['purust_core', 'Purs_ChildCallReuse']
