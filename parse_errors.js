...
const ffiMap = {
    ...
    'Data_Ord_Down': 'purust-prelude/src/Data/Ord/Down.rs', // Might be wrong package, but just put somewhere
    ...
    'Test_Liveness': 'purust-assert/src/Test/Liveness.rs', // Mock path
...
function mapPrefixToPath(prefix) {
    ...
    if (parts.length >= 2) {
        // e.g. Data_Array_foo -> purust-arrays/src/Data/Array.rs
        const pkg = parts[0].toLowerCase() === 'data' ? 'purust-prelude' : \`purust-\${parts[1].toLowerCase()}\`;
        // Just put them in a generic file for now if not found
...
for (const fn of missingFns) {
    ...
    // Special handling if prefix doesn't match
    let p = prefix;
    while(p.length > 0) {
        if (ffiMap[p]) {
            prefix = p;
